...
drawMarkers=function(K,frameIdx,ov){
  ...
  if(child){
    for(let e of ov.ranks){
      ...
      ctx.strokeStyle=e.uncert?"#9e9e9e":COLORS[r];ctx.lineWidth=3;
      ...
      ctx.fillStyle=e.uncert?"#9e9e9e":COLORS[r];
      ctx.beginPath();ctx.arc(cx,byBadge,8,0,7);ctx.fill();
      ctx.fillStyle="#fff";ctx.font="bold 9px monospace";
      ctx.fillText(LETTERS[r]+(e.uncert?"?":""),cx-(e.uncert?5:3),byBadge+3);
